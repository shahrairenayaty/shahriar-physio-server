...
describe("testing patient modeld",()=>{
  let physio
  let patient
  let visit;
  let exercise;
  ...
  beforeEach((done)=>{
    ...
    Promise.all([patient.save(),physio.save(),visit.save(),movement.save()])
      .then(()=>{
        ...
        visit.result.exercises.push(exercise);
        visit.result.exercises.push(exercise2);
        visit.save()
          .then(()=>{
            //console.log("yes");
            done();
  ...
  it("login patient successfull",(done)=>{
    request(app)
    ...
    .end((err,res)=>{
      ...
      assert(res.body.myServer.id[0]===20006);
      assert(res.body.myServer.id[1]===20007);
      assert(res.body.myServer.id[2]===20010);
      assert(res.body.myServer.id[3]===8585);
      assert(res.body.myServer.visits[0]._id==visit._id)
      done();
    })
  ...
  it("updatePatientPersonalInfo successful",(done)=>{
    request(app)
      .put("/api/patient/"+patient.token.mainToken)
      .send({
        "myInfo":{
          	"patient":{
          		"name":"ali",
      ...
      .end((err,res)=>{
        //console.log(JSON.stringify(res.body,undefined,2));
        assert(res.body.myServer.id[0]===20000);
        assert(res.body.myServer.id[1]===20008);
        assert(res.body.myServer.id[2]===20009);
        assert(res.body.myServer.id[3]===20017);
        assert(res.body.myServer.id[4]===20010);
        assert(res.body.myServer.id[5]===8585);
        assert(res.body.myServer.patient!=undefined);
  ...
  it("get visits of patient successful",(done)=>{
    request(app)
      .get("/api/patient/"+patient.token.mainToken)
      .end((err,res)=>{
        //console.log(JSON.stringify(res.body,undefined,2));
        assert(res.body.myServer.id[0]===20000)
        assert(res.body.myServer.id[1]===20010)
  ...
  it("makeTrueIsdoneOfExercise",(done)=>{

    request(app)
      .post("/api/patient/exercis/"+patient.token.mainToken)
      .send({
        "myInfo":{
          "visit":{
            "_id":visit._id,
          ...
          "exercise":{
            "_id":visit.result.exercises[0]._id
          },
          "condition":true
        }
      })
