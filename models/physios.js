...
PhysioSchema.statics.getPhysioIdByToken = function(token) {
    return Physio.findOne({"token.mainToken":token}).select({_id:1})
};
PhysioSchema.statics.getPhysioByToken = function(token) {
    return Physio.findOne({"token.mainToken":token})
};
PhysioSchema.statics.updateByToken = function(token,physioInfo) {
    return Physio.findOneAndUpdate({"token.mainToken":token},physioInfo,{runValidators:true,new:true})
};

PhysioSchema.statics.findPhysioByWorkNumber = function(number) {
    return Physio.findOne({"number.title": "مطب","number.mainNumber": number})//.select({password:1})
};
PhysioSchema.statics.totalUpdateById = function(id,physioInfo) {
    return Physio.findOneAndUpdate(id,physioInfo,{runValidators:true,new:true})
};
PhysioSchema.methods.addPatient = function(patient) {
    const newPatient = {patient:patient._id}
    return this.model('physio').findByIdAndUpdate(this._id,{$push:{"patients":newPatient}},{ new: true, upsert:true,runValidators:true })
      .populate({path:"patients.patient",model:"patient"})
};
PhysioSchema.statics.getPhysioInfosByIdForSelf = function(id) {
    return Physio.findOne({"_id":id}).select({password:0})
      .populate({path:"patients.patient",model:"patient"})
};

PhysioSchema.statics.updatePatientCondition = function(physioId,patientId,condition){
  return Physio.findOneAndUpdate({_id:physioId,"patients.patient":patientId},{$set:{"patients.$.condition":condition}},{ new: true, upsert:true,runValidators:true })
